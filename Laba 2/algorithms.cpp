...
void ListFiles(const std::wstring& directory)
{
    ...
    do {
        const std::wstring fileName = findFileData.cFileName; // получаем имя файла
...
{
    DWORD result = AddFontResourceEx(fontFilePath, FR_PRIVATE, 0); // добавляет ресурс шрифта 
...
void ListFonts(std::set<std::wstring>& fontNames) {
    ...
    if (hdc) {
        ...
        ReleaseDC(NULL, hdc); // Освобождаем контекст устройсва
    }
}
